...
function writeDownload(data, site, serviceId){
    ...
    var exportFileName = site + "_stucktransfers(" + month +"-"+d.getDate()+"-"+d.getFullYear()+ ").csv";
...
function makeGraphTrace(data, site, replicaname){
	...
	try{
	    ...
	    //correcting for strange appearance of 0s at the end of replica.ratio -- always 8 0s
	    replica.total.length = replica.total.length - 8;
	    replica.copied.length = replica.copied.length - 8;
	    replica.ratio.length = replica.ratio.length - 8;
